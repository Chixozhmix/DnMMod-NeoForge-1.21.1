package net.chixozhmix.neodnm.entity.spell.tall_the_dead;
...
public class TallTheDeadRenderer extends EntityRenderer<tallTheDead> {
    public TallTheDeadRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
    }

    @Override
    public boolean shouldRender(tallTheDead pLivingEntity, Frustum pCamera, double pCamX, double pCamY, double pCamZ) {
        return true;
    }

    @Override
    public void render(tallTheDead entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int light) {
        poseStack.pushPose();
        ...
        for (int i = 0; i < 4; ++i) {
            int r = (int) (Mth.clamp(0.1F * f * f, 0.0F, 1.0F) * 255.0F);
            int g = (int) (Mth.clamp(0.7F * f, 0.0F, 1.0F) * 255.0F);
            int b = (int) (Mth.clamp(0.05F * f * f, 0.0F, 1.0F) * 255.0F);
            int a = 255;
            Matrix4f poseMatrix = poseStack.last().pose();

            // Первая группа вершин (половинный радиус)
            ...
            // Вторая группа вершин (четвертной радиус)
            int color = RenderHelper.colorf(
                    Mth.clamp(0.05F * f * f, 0.0F, 1.0F),
                    Mth.clamp(0.8F * f, 0.0F, 1.0F),
    ...
    @Override
    public ResourceLocation getTextureLocation(tallTheDead contagionRay) {
